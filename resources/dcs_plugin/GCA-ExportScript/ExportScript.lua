...
--ExportScript.Version.ExportScript = "0.0.0"
-- Simulation id
...
dofile(lfs.writedir()..[[Scripts\GCA-ExportScript\lib\Tools.lua]])
...
-- For example:
	...
	udp = socket.udp()
	udp:settimeout(0)
	udp:setsockname('*', listen_port)

	--if default_output_file then
	--	default_output_file:write(string.format("t = %.2f, Started listening on port %u\n", t, listen_port))
	--end
...
-- 2) Socket
--	sender.try(c:send("quit")) -- to close the listener socket
--	c:close()
   udp:close()
...
function LuaExportActivityNextEvent(t)
	...
	--Make sure we always has the latest data!!!
	data, msg_or_ip, port_or_nil = udp:receivefrom()
	repeat
		latest_data = data
		latest_msg_or_ip = msg_or_ip
		latest_port_or_nil = port_or_nil
		data, msg_or_ip, port_or_nil = udp:receivefrom()
	until(not data)

	data = latest_data
	msg_or_ip = latest_msg_or_ip
	port_or_nil = latest_port_or_nil
	...
	if data then
		...
		for k,v in pairs(o) do
			...
			if (v.Type.level1 == 1) then	-- if the object is an flying one
				...
				plane_z = plane_xyz_coords.z

				if (((plane_x-thr_xyz_coords.x)^2 + (plane_z-thr_xyz_coords.z)^2) < 1600000000) then	-- if object less than 21 nautical miles (~40000 meters) from thr

					s_plane = ','..tostring(k)..','..string.format("%.1f", plane_x)..','..string.format("%.1f", v.LatLongAlt.Alt)..','..string.format("%.1f", plane_z)
					s_final = s_final..s_plane
				end
			end
		end
...
--  if default_output_file then
	...
	--	default_output_file:write(string.format("t = %.2f, ID = %d, name = %s, country = %s(%s), LatLongAlt = (%f, %f, %f), heading = %f\n", t, k, v.Name, v.Country, v.Coalition, v.LatLongAlt.Lat, v.LatLongAlt.Long, v.LatLongAlt.Alt, v.Heading))
		...
		--udp_listener:sendto(string.format("t = %.2f, ID = %d, name = %s, country = %s(%s), LatLongAlt = (%f, %f, %f), heading = %f\n", t, k, v.Name, v.Country, v.Coalition, v.LatLongAlt.Lat, v.LatLongAlt.Long, v.LatLongAlt.Alt, v.Heading), msg_or_ip, send_port)
	--end

	--if default_output_file then
	--	default_output_file:write(string.format("t = %.2f, nbr_of_objs = %d\n", t, count))
	--end
